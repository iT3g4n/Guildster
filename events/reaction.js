...
module.exports = {
  ...
  run: async (reaction, user) => {
    ...
    if (reaction.emoji.name == "🎫" && message.channel.id === result.Tickets) {
      ...
      if (db.has(`TICKET: ${user.id}`))
        return (
          reaction.message.channel
            .send(
              `Sorry <@${user.id}>. You already have a ticket open! Please wait for the time to be over.`
            )
      ...
      const m = await reaction.message.guild.channels.create(
        `${user.id}-ticket`,
        {
          type: "text",
      ...
      const collector = m.createMessageCollector(
      ...
      collector.on("collect", async (message) => {
        const ticketembed = new MessageEmbed()
          .setDescription(
            `**Suggestion by <@${message.author.id}>**\n\n**Suggestion**\n${message.content}`
          )
          .setColor("RANDOM");
        const ticketid = "739480654109999185";

        let m = await bot.channels.cache
